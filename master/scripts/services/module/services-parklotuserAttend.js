...
appService.factory("parklotuserAttendService", ['$q', '$http', 'Setting', '$log','sessionCache',
	function($q, $http, Setting, $log,sessionCache) {
		...
		var reqPagination = function(data) {
			...
			$http({
				url: Setting.lotUrl.PARKLOTUSERATTEND.PARKLOTUSERATTEND_PAGINATION,
				method: 'GET',
				params: reqData
    ...
    let attendenceReport = function (data, type) {
        ...
        if (type === 1) {
            reqData.start = 0;
            reqData.length = 10;
            url = Setting.lotUrl.PARKLOTUSERATTEND.PARKLOTUSERATTEND_EXCEL_COUNT;
        } else if (type === 2) {
            reqData.start = data.start;
            reqData.length = data.length;
            url = Setting.lotUrl.PARKLOTUSERATTEND.PARKLOTUSERATTEND_EXCEL;
        }
