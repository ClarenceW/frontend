...
appService.factory("lotparkingService", ['$q', '$http', 'Setting', '$log',
	function($q, $http, Setting, $log) {
		...
		var reqPagination = function(data) {
			...
			reqData.stopTimeMin = data.stopTimeMin;
			reqData.stopTimeMax = data.stopTimeMax;
			...
			$http({
				url: Setting.lotUrl.LOTPARKING.LOTPARKING_PAGINATION,
				method: 'GET',
				params: reqData
    ...
    var reqDetail = function (data) {
        ...
        var deferred = $q.defer();
        $http({
            url: Setting.lotUrl.LOTPARKING.LOTPARKING_DETAIL,
            method: 'GET',
            params: reqData
        }).success(function (resp) {
