...
angular.module('parkingApp').controller('parkingnumListCtrl', ['$rootScope', '$scope', '$http', '$state', '$uibModal', 'growl', 'customerService', 'parkingnumReportService', 'StateName', 'sessionCache','$filter','$compile','Setting','excelService','parklotsService','$timeout',
	function($rootScope, $scope, $http, $state, $uibModal, growl, customerService, parkingnumReportService, StateName, sessionCache,$filter,$compile,Setting,excelService,parklotsService,$timeout) {

		var indexId = "#parkingnum-index";
		var tableId = indexId + " #parkingnum-table";
    ...
    $scope.formData.endTime = new Date(yesterday);
    ...
    parklotsService.reqList().then(function(resp) {
      ...
      $timeout(function() {
		...
		function initTable(index) {
          ...
          data.index = index;
					parkingnumReportService.reqPagination(data).then(function(resp) {
						callback(resp)
					}, function(msg) {
    ...
    $scope.selDate = function(index){
     ...
     $scope.downloadExcel = function () {
      ...
      reqData.index = selDateFlg;
			parkingnumReportService.reqList(reqData).then( resp => {
				if(resp && resp.length > 0){
					...
					excelData = resp.map(function (item) {
						return {
							...
							'运营泊位总数': item.berthNum,
							'总停车次数': item.tccs,
							'15分钟内免费次数': item.tccsFree,
							'包月车停车次数': item.tccsVip,
							'平均停车时长（小时/次）': item.avgPkTime,
						}
					});
					let excelName = '停车场停车次数报表';
