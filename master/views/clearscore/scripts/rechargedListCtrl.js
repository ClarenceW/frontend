...
angular.module('parkingApp').controller('RechargedListCtrl', ['$scope', 'growl', 'customerService', 'clearScoreRechargedService', '$filter', 'Setting',
	function ($scope, growl, customerService, clearScoreRechargedService, $filter, Setting) {
		var indexId = "#list-recharged-index";
		var tableId = indexId + " #list-recharged-table";
		var oTable = null;
		$scope.formData = {};
		...
		function initTable() {
			oTable = $(tableId).dataTable($.extend(UcUtils.getTableDefaultOptions(), {
				"ajax": function (data, callback, settings) {
					...
					clearScoreRechargedService.reqInPagination(data).then(function (resp) {
						callback(resp)
					}, function (msg) {
