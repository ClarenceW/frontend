...
angular.module('parkingApp').controller('ScoreListCtrl', ['$scope', 'growl', 'customerService', 'clearScoreScoreService', '$filter', 'Setting',
	function ($scope, growl, customerService, clearScoreScoreService, $filter, Setting) {
		var indexId = "#list-score-index";
		var tableId = indexId + " #list-score-table";
		var oTable = null;
		$scope.formData = {};
		...
		function initTable() {
			oTable = $(tableId).dataTable($.extend(UcUtils.getTableDefaultOptions(), {
				...
				"columns": [
					...
					{ "data": "accountPeriod" },
					{ "data": "paymentChannelClear" },
					{ "data": "collectionCompanyClear" },
					{ "data": "clearCycle" },
					{ "data": "ClearTime" }
				],
