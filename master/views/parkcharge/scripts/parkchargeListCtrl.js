...
angular.module('parkingApp').controller('parkchargeListCtrl', ['$rootScope', '$scope', '$http', '$state', '$uibModal', 'growl', 'customerService', 'parkchargeService', 'StateName', 'sessionCache','$filter',
	function($rootScope, $scope, $http, $state, $uibModal, growl, customerService, parkchargeService, StateName, sessionCache,$filter) {

		var indexId = "#parkcharge-index";
		var tableId = indexId + " #parkcharge-table";
    ...
    if ($state.params.searchConditions){
       ...
       if ($state.params.searchConditions.startVioTime) {
		...
		function initTable() {
			oTable = $(tableId).dataTable($.extend(UcUtils.getTableDefaultOptions(), {
				"ajax": function(data, callback, settings) {
					...
					parkchargeService.reqPagination(data).then(function(resp) {
						callback(resp)
					}, function(msg) {
          ...
          { "data": "carouttime" },
          { "data": "order" },
          { "data": "ordercost","render":function(data,type,full){
						data = $filter("currency")(data);
						return data;
					} },
					{ "data": "parkcost","render":function(data,type,full){
						data = $filter("currency")(data);
						return data;
					} },
					{ "data": "cartype" },
          { "data": "paycost","render":function(data,type,full){
						data = $filter("currency")(data);
						return data;
					} },
          { "data": "couponcost" ,"render":function(data,type,full){
						data = $filter("currency")(data);
						return data;
					}},
          { "data": "payway" }
		...
		$scope.toReset = function() {
			...
			$scope.formData.payTypt = "";
			$scope.formData.memberType = "";
			reload();
		};
		...
		$scope.toDelete = function() {
			...
			if(count === 1) {
				customerService.modalInstance().result.then(function(message) {
					...
					parkchargeService.reqDelete(data).then(function(msg) {
