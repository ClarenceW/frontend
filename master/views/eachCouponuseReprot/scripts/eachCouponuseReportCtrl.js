...
angular.module('parkingApp').controller('eachCouponuseReportCtrl', ['$rootScope', '$scope', '$http', '$state', '$uibModal', 'growl', 'customerService', 'eachcouponuseReportService', 'StateName', 'sessionCache','$filter','$compile','Setting','excelService',
	function($rootScope, $scope, $http, $state, $uibModal, growl, customerService, eachcouponuseReportService, StateName, sessionCache,$filter,$compile,Setting,excelService) {

		var indexId = "#eachcouponuseReport-index";
		var tableId = indexId + " #eachcouponuseReport-table";
    ...
    $scope.couponTypeOption=sessionCache.getDictsByLookupName("COUPON_RULE_TYPE");
		...
		function initTable(index) {
			oTable = $(tableId).dataTable($.extend(UcUtils.getTableDefaultOptions(), {
				"ajax": function(data, callback, settings) {
					if($scope.formData.activityName) {
						data.activityName = $scope.formData.activityName;
					}
          ...
          data.index = index;
					eachcouponuseReportService.reqPagination(data).then(function(resp) {
						callback(resp)
					}, function(msg) {
    ...
    $scope.selDate = function(index){
     ...
     $scope.downloadExcel = function () {
        ...
        title = '报表月份';
			...
			if($scope.formData.activityName) {
        reqData.activityName = $scope.formData.activityName;
      }
      if($scope.formData.couponType) {
        reqData.couponType = $scope.formData.couponType;
      }
      if($scope.formData.startTime) {
      ...
      reqData.index = selDateFlg;
			eachcouponuseReportService.reqList(reqData).then( resp => {
				if(resp && resp.length > 0){
					...
					excelData = resp.map(function (item) {
						return {
							[title]: item.date,
							'优惠活动编号': item.activityId,
							'优惠类型': item.couponType,
							'优惠活动名称': item.activityName,
							'活动开始时间': item.releaseTime,
							'领取次数': item.receiveNum,
							'使用次数': item.usedNum,
							'优惠券总额（元）': item.couponMoney,
							'实际优惠金额（元）': item.actualcouponMoney,
						}
					});
					let excelName = '各优惠券领用报表';
