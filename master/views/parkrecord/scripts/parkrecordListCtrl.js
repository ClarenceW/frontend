...
angular.module('parkingApp').controller('parkrecordListCtrl', ['$rootScope', '$scope', '$http', '$state', '$uibModal', 'growl', 'customerService', 'parkrecordService', 'StateName', 'sessionCache','$filter',
	function($rootScope, $scope, $http, $state, $uibModal, growl, customerService, parkrecordService, StateName, sessionCache,$filter) {

		var indexId = "#parkrecord-index";
		var tableId = indexId + " #parkrecord-table";
		...
		function initTable() {
			oTable = $(tableId).dataTable($.extend(UcUtils.getTableDefaultOptions(), {
				"ajax": function(data, callback, settings) {
					...
					parkrecordService.reqPagination(data).then(function(resp) {
						callback(resp)
					}, function(msg) {
		...
		$scope.toDelete = function() {
			...
			if(count === 1) {
				customerService.modalInstance().result.then(function(message) {
					...
					parkrecordService.reqDelete(data).then(function(msg) {
