...
angular.module('parkingApp').controller('berthOperateReportCtrl', ['$rootScope', '$scope', '$http', '$state', '$uibModal', 'growl', 'customerService', 'berthOperateService', 'StateName', 'sessionCache','$filter','$compile','Setting','excelService','parklotsService','$timeout',
	function($rootScope, $scope, $http, $state, $uibModal, growl, customerService, berthOperateService, StateName, sessionCache,$filter,$compile,Setting,excelService,parklotsService,$timeout) {

		var indexId = "#berth-report-index";
		var tableId = indexId + " #berth-report-table";
    ...
    parklotsService.reqList().then(function(resp) {
      ...
      $timeout(function() {
		...
		function initTable(index) {
          ...
          data.index = index;
					berthOperateService.reqPagination(data).then(function(resp) {
						callback(resp)
					}, function(msg) {
    ...
    $scope.selDate = function(index){
     ...
     $scope.downloadExcel = function () {
      ...
      reqData.index = selDateFlg;
			berthOperateService.reqList(reqData).then( resp => {
				if(resp && resp.length > 0){
					...
					excelData = resp.map(function (item) {
						return {
							...
							'泊位总数': item.berthNum,
							'泊位利用率': item.usedRate,
							'泊位周转率（次）': item.turnoverRate,
						}
					});
					let excelName = '泊位运营报表';
