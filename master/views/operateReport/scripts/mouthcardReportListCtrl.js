...
angular.module('parkingApp').controller('mouthcardListCtrl', ['$rootScope', '$scope', '$http', '$state', '$uibModal', 'growl', 'customerService', 'mouthcardReportService', 'StateName', 'sessionCache','$filter','$compile','Setting','excelService','parklotsService',
	function($rootScope, $scope, $http, $state, $uibModal, growl, customerService, mouthcardReportService, StateName, sessionCache,$filter,$compile,Setting,excelService,parklotsService) {

		var indexId = "#mouthcardreport-index";
		var tableId = indexId + " #mouthcardreport-table";
    ...
    $scope.tempBeginData = {};
          ...
          if($scope.formData.startTime  && $scope.formData.endTime) {
            if($scope.formData.startTime > $scope.formData.endTime) {
                ...
                return;
					...
					mouthcardReportService.reqPagination(data).then(function(resp) {
						callback(resp)
					}, function(msg) {
     ...
     /****   excel导出代码开始     ******/
        ...
        $scope.toExport = function () {
            ...
            mouthcardService.mouthcardReport(data, 1).then(function (resp) {
              if (resp.code === Setting.ResultCode.CODE_SUCCESS) {
                if (resp.data) {
                  ...
                  if ($scope.totalCount >= $scope.baseNumber) {
                    ...
                    mouthcardService.mouthcardReport(data, 2).then(function (resp) {
        ...
        function downloadExcel(data, excelItem, index) {
            ...
            excelData = data.map(function (item) {
                return {
                    ...
                    '月卡充值金额（元）': item.statisticsMoney,
                }
            });
            let excelName = '包月车充值记录';
