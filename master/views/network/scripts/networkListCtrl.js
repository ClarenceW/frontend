...
angular.module('parkingApp').controller('networkListCtrl', ['$scope', 'growl', 'customerService', 'networkMonitorService', '$filter','Setting',
	function($scope, growl, customerService, networkMonitorService, $filter,Setting) {
		var indexId = "#network-index";
		var tableId = indexId + " #network-table";
		var oTable = null;
		$scope.formData = {};
		...
		function initTable() {
			oTable = $(tableId).dataTable($.extend(UcUtils.getTableDefaultOptions(), {
				"ajax": function(data, callback, settings) {
					...
					networkMonitorService.reqPagination(data).then(function(resp) {
						callback(resp)
					}, function(msg) {
